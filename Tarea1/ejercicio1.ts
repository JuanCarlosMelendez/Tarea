let entrada: string = 'A man, a plan, a canal. Panama';

const palindromeChecker = ( entrada: string ) =>  {
    let entradaFiltrada: string = entrada.replace(/[0-9]+/g, "").replace(/[^\w]/gi, '') .toLowerCase();
    let entradaInvertida: string = entradaFiltrada.split('').reverse().join('');
